...
function tamanoDePaso(t0,tf,N)
    ...
    return (tf-t0)/N #Aqui se usa la expresión definida por el profesor.

end

# ╔═╡ 4923fac7-5cd3-478b-b01f-72def5f94217
tamanoDePaso(2,2.5,10)
...
# ╔═╡ b05fc913-1f6c-42d0-917e-84f4b89d071a
arregloUniforme(2,2.5,10,tamanoDePaso(2,2.5,10))

# ╔═╡ a38a94d4-a744-4077-aa47-49f405c41c70
arregloUniforme(2,2.5,-10.1,tamanoDePaso(2,2.5,-10.1))
...
function paso_euler(ti, xti, g, h)
    return xti+h*g(ti, xti) #Aquí se utiliza la expresión x_ti que deduce el profesor 
end
...
function euler(g, xt0,t,t0,tf,N)
    xt=zeros(1,N+1) #Aquí se guardan los xt
    xt[1]=xt0 #Se guarda xt0 en la primer entrada xt
    for i in 2:N+1
        xt[i]=paso_euler( t[i-1], xt[i-1], g, tamanoDePaso(t0,tf,N))#Aquí se obiene xti a partir de su término anterior
    end
    return xt 
end

# ╔═╡ 2c1b489e-ba49-43d7-be19-e134e723cff2
begin
#G=function F(t,x)
#return 0.1*(x)^(1/2)+0.4*t^2
#end
...
# ╔═╡ 2e0bda3b-90ec-4938-abc8-4b78430ca02a
begin
T=arregloUniforme(0,5,2000,tamanoDePaso(0,5,2000))
euler(G, 1,T,0,5,2000)
end
...
Si $x'(t)=g(t)$, entonces con en el método de Euler estamos hallando el valor de $x(t_i)=\int_{t_0}^{t_i} g(s)ds + x(t_0)\approx x(t_{i-1}) + hg(t_{i-1})$. Yo defino mi función $g(t)=t^3$ con condición inicial $x(0)=3$, entonces $x(t_i)=\int_{0}^{t_i} s^3ds + 3\approx x(t_{i-1}) + (h)(t_{i-1})^3$, en este caso particular podemos calcular $x(t_i)=(t_i)^4/4+3$.
...
#Definimos nuestra función g(t)=H(t,x)=t^3 con condición inicial x(0)=3
g=function H(t,x)
return t^3
end


# ╔═╡ de9e98a0-0a04-49a3-afa7-23d0195fa9e8
begin
TA=arregloUniforme(0,5,100000,tamanoDePaso(0,5,100000))
euler(g, 3,TA,0,5,100000)
end
...
begin
#Definimos nuestra función L(t)=K(t,x)=x con condición inicial x(0)=5
L=function K(t,x)
return x
end
end

# ╔═╡ 6d411c30-0ab2-4a45-806f-5dde04c5b3dd
@bind N Slider(1:100, default=0.5)

# ╔═╡ fe661318-8489-4a46-9ce9-f2af29d74894
begin

a=3
t_g=arregloUniforme(-a,a,N,tamanoDePaso(-a,a,N))
xt_g=euler(L, 5,t_g,-a,a,N)
end

# ╔═╡ 21c80153-30c5-43a0-9d9f-532864cf5c81
scatter(t_g, xt_g, color = "red",legend=false )
...
julia_version = "1.8.5"
manifest_format = "2.0"
project_hash = "5c93df484335c1cc762e4f660f11c4d2f6982dca"
