...
# ╔═╡ db2b235c-f2a1-4b43-942c-b62c91613954
using Plots, PlutoUI
...
# ╔═╡ cca2f73d-085c-42b9-b1c9-1e99b4182cd7
begin
y=function H(x)
   return x^2-3
end
...
begin
function newtonRecursiva(f,f′,x0,n)#Aquí defino mi función con sus respectivos parámetros.
    
    if n <= 0 #Cuando n haya disminuido hasta n=0, devuelve xn, que puede ser x0 si n=0 o xn si n>0.  
    return x0
    else
    return newtonRecursiva(f,f′,x0-f(x0)/f′(x0),n-1) #Aquí se disminuye una unidad a n, y se vuelve a evalura a la función otras                                                      #n-1 veces
    end
end
end

# ╔═╡ 898bc44e-c10c-4f27-853c-a855ab632ffe
newtonRecursiva(y,z,1,3)
...
function newtonDistancia(f,f′,x0,eps)
    if eps <=0 #Aquí le pregunto si el épsilon es menor que cero, si es así, entonces que muestre un mensaje de error.
        return println("Error: Tu épsilon no puede ser menor o igual a cero.")
	else #Si el épsilon es mayor a cero, que pruebe si  dos aproximaciones consecutivas 
	     #del método tengan una distancia menor a épsilon y vaya incrementado una unidad a n.
        n=1
        while abs(newton(f,f′,x0,n)-newton(f,f′,x0,n+1))>eps #Aquí pruebo con distintos n, hasta que se cumple que la distancia
			                                                 #entre las aproximaciones es menor que el épsilon
            n=n+1
        end
...
# ╔═╡ 01572d7d-2993-4e83-95f9-7f60751c8c16
...
function derivadaNumérica(f,x,h)
   return h*(f(x+1/h)-f(x)) #Aquí uso la aproximación de la derivada
end


# ╔═╡ 9f9c57a9-0661-44ee-94f7-e3f698b42d56
m=function K(a) #Aquí defino mi función que quiero calcularle su derivada.
    a^3
end

# ╔═╡ 4fb4c274-3286-4718-acdf-0aac089f0ac6
derivadaNumérica(m,3,1000)
...
begin
function newtonDN(f,x0,n,h) #Aquí defino mi función con sus respectivos parámetros.
    for i in 1:n 
    x0=x0-f(x0)/derivadaNumérica(f,x0,h) #Aquí se va actualizando el valor de x0 por cada iteración, para calcular la derivada
		                                 # se usa la función derivadaNumérica. 
    end
	return x0
end
end

# ╔═╡ 283cc473-f704-41df-8497-6795b8d7d167
newtonDN(y,1,30,1000)
...
# ╔═╡ 10b2bd9b-6b4b-4c34-805d-3d9da97f2d7d
begin
function Arraynewton(f,x0,n,h) #Aquí defino mi función con sus respectivos parámetros.
A=[]
push!(A, x0 )
    for i in 1:n 
    x0=x0-f(x0)/derivadaNumérica(f,x0,h) #Aquí se va actualizando el valor de x0 por cada iteración, para calcular la derivada
...
# ╔═╡ 121e0999-a126-47d3-bdcc-37ef801127a9
begin
function grafNewton(T,f) #Esta función se encarga de evaluar cada xi en la función, para que se grafique.
FA=[] #Este es un arreglo donde se guardaran los f(x_i) para graficarlos.
for i in 1:length(T)
    push!(FA, f(T[i])) #Aquí se guarda el valor de f(x_i) por cada iteración en A.
end
    plot(T,FA, legend=false)#Aquí se grafica la función con lineas.
    scatter!(T,FA)#Aquí se grafica la función con puntos.
    xlabel!("eje x")
    ylabel!("eje y")
end
end

# ╔═╡ f0836f61-6945-4e63-b002-ba659daa67cf
@bind x0 Slider(-10:0.1:10, default=0.1)

# ╔═╡ 399cb9bf-98b1-4257-94a5-a51382c27d8c
@bind h Slider(1000:1000:1000000, default=1000)

# ╔═╡ c4a0afed-d3fd-408d-a0bc-38a408c2fa17
@bind n Slider(1:1000, default=1)

# ╔═╡ f8d7742c-3a22-44e0-8fa5-4b357af5c40c
grafNewton(Arraynewton(f,x0,n,h),f)
...
grafNewton(Arraynewton(z,x0,n,h),z)

# ╔═╡ adeae8cb-e7f0-4f60-b04c-33ee33abf371
grafNewton(Arraynewton(m,x0,n,h),m)
